...
import type { CodeforcesHandle, CsesUserNumber, Email, PasswordHash, SolutionId, UserConfig, UserUId, UserName, UserPassword } from "$lib/models/user";
...
export async function initDatabase(): Promise<DatabaseContainers> {
    ...
    return {
        ...
        submissions: await getContainer(db, 'submissions') satisfies UserSubmissionsContainer,
...
const emailToId = (email: Email) => createHash('sha256').update(email).digest('hex') as UserUId;
...
export const getUserConfig = async (db: DatabaseContainers, id: UserUId) => await getUserItem(db.configs, id) as UserConfig | null;
const checkIfRegistered = async (db: DatabaseContainers, id: UserUId) => Boolean(await getUserItem(db.configs, id));
...
export async function getUserSubmissions(db: DatabaseContainers, id: UserUId | null): Promise<UserSubmissions | null> {
    if (!id) return null;
